...
// Stream: 연속된 데이터를 효율적으로 처리할 수 있는 추상화된 방법
...
//  ㄴ 참고) 중개 오퍼레이션과 종료 오퍼레이션의 가장 큰 차이는 Stream의 반환 여부이다. ( 중개 오퍼레이션은 스트림을 리턴하고, 종료 오퍼레이션은 스트림이 아닌 다른 타입을 리턴한다. (스트림을 리턴하지 않으면 모두 종료 오퍼레이션이다.) )
//
// Stream 파이프라인 정의
